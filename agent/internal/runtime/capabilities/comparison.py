...
def _compare_photo_periods(params: dict, ctx: rt_registry.RunContext) -> rt_state.Observation:
    ...
    earlier_ids = [str(pid) for pid in params.get("earlier_ids") or groups.get("earlier") or []]
    later_ids = [str(pid) for pid in params.get("later_ids") or groups.get("later") or []]
    if not earlier_ids or not later_ids:
        return rt_state.Observation(
            rt_state.OBS_ERROR, "跨期对比需要两组非空照片 ID",
...
COMPARE_PHOTO_PERIODS = rt_registry.Capability(
    ...
    parameters={
        "earlier_ids": {"type": "list", "description": "较早时期的照片 ID（已标记期别的检索可省略）", "required": False},
        "later_ids": {"type": "list", "description": "较近时期的照片 ID（已标记期别的检索可省略）", "required": False},
    },
    run=_compare_photo_periods,
    level="skill",
    applicable_when="已收集两组跨期照片证据，且目标需要综合结论时",
