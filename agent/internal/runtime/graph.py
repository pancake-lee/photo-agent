"""
    ...
    runtime 核心模块中（state / budget / completion / registry / capabilities）：
        - decide  模型在语义不确定性中选择下一动作和参数（唯一的 LLM 决策点）
        - execute 程序执行能力（参数校验 + 注册表查找）
        - reduce  程序把观察归约进 TaskState
        - check   程序做完成检查与预算停止判定，决定回环或收尾
"""
...
def _decide_node(state: RuntimeGraphState, config: lc_runnables.RunnableConfig) -> dict:
    ...
    response = llm.invoke([
        lc_messages.SystemMessage(content=_RUNTIME_DECIDE_SYSTEM),
    ...
    parsed = rt_capabilities.extract_json_dict(str(response.content)) or {}
    decision = {
...
def _finish_node(state: RuntimeGraphState, config: lc_runnables.RunnableConfig) -> dict:
    ...
    photos: list[dict] = []
    for photo in rt_capabilities.fetch_photos_batch(cfg, state["task"].artifacts.selected_ids):
        pid = photo.get("id", "")
        photos.append({
            "photo_id": pid,
...
def run_runtime(
    ...
    result = _get_runtime_graph().invoke(initial, {
        "configurable": {
            "cfg": cfg,
            "llm_callbacks": callbacks,
            "tracer": tracer,
