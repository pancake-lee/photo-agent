...
from utils.token_tracker import TokenTracker, load_prices, validate_model_prices


class TokenTrackerTest(unittest.TestCase):
    def test_uses_yuan_per_million_tokens(self):
        with tempfile.TemporaryDirectory() as directory:
            tracker = TokenTracker(pathlib.Path(directory, "tokens.db").as_posix(), {"m": {"input": 2, "output": 4}})
    ...
    def test_rejects_invalid_price_configuration_and_missing_model(self):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml") as file:
            ...
            with self.assertRaises(ValueError):
                load_prices(file.name)
        with self.assertRaisesRegex(ValueError, "缺少"):
            validate_model_prices({"m": {"input": 1}}, "m", "missing")
