...
- 用户可从问题清单定位全部异常，也可从任意照片详情完成“诊断 → 修复 → 验证”。
...
照片管理增加“AI 资产问题”工作区，可按状态、阶段、问题类型、时间线和文件格式筛选。它需要同时覆盖：

- 原图不可读、预览图不可读、VLM 请求缩略图不可读或来源身份不匹配；
- VLM 调用失败、返回空结果、结构不符合约定；
- 描述内容触发质量规则，例如摄影原图却被描述成故障彩条、截图或空白画面；
- 描述已变更但旧向量仍在、向量写入失败或向量来源已过期；
- 历史数据审计发现的疑似异常。

列表必须展示问题为何被识别、影响哪个阶段和最近一次处理结论；用户可进入原图、预览图、VLM 请求缩略图和 AI 描述进行对照。批量操作只面向用户已确认的目标，操作前明确影响数量与会触发的后续阶段。
...
- 最近一次成功、失败或质量拒绝的原因；
- 与当前照片状态匹配的操作：重新生成描述、重新生成向量、从描述开始完整修复、标记为误报或排除。

“重新生成描述”会使旧向量变为过期；用户不需要猜测是否还需单独操作。详情页应明确下一步并在完成后刷新最终健康结论。

### 4.4 队列与结果反馈

VLM 和 Embedding 队列的完成反馈必须区分成功、失败、待复核、跳过和取消；不能用“completed”覆盖无效产物。用户离开页面或服务重启后，最近任务的总数、结果分布和失败清单仍可查询。

队列结束时应提供明确收口：全部健康、存在待复核/失败并跳转问题清单、或因用户停止而留下未处理项。
...
- 语义异常检测检查高风险模式。首批规则覆盖本次已证实的故障彩条、纯色/空白、截图式错误描述等；规则命中进入“待复核”，而不是擅自删除照片或把模型调用算作失败。
...
### 6.2 采用“自动隔离 + 人工确认”，不自动删除异常照片

自动删除风险高，纯人工排查又无法阻止污染扩散。异常资产先退出检索并进入待复核，用户可查看证据后重新生成、确认误报或排除。这符合照片库的资产安全性，也避免把模型不确定性当作事实。
...
- 基于确定性校验和首批高风险语义规则，产出问题清单；未能自动判定的记录进入待复核。
...
- 人为制造调用失败、空响应、无效预览/请求缩略图和高风险描述时，均能在任务结果、照片详情和问题清单中看到一致原因。
- 质量异常照片不进入或立即退出默认照片检索、组图检索和对话上下文。
- 用户可从任一照片详情查看 AI 描述和向量状态，并执行与当前状态匹配的重新生成或完整修复操作；各入口行为一致。
- 问题清单能筛出全部已知异常资产，并支持定位到原图、预览图、VLM 请求缩略图、描述和最近处理履历。
- 黄金用例运行会揭示关联照片的资产健康；在 `DSC_1813.jpg` 修复后，能保存该用例和对应对话的前后对比证据。
- 队列完成报告包含成功、失败、待复核、跳过/取消的数量及可访问明细；重启服务后最近任务结果仍可查询。
