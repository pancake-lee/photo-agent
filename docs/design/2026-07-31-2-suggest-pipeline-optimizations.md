...
- [ ] `GET /api/suggest/history` 返回的数据与当前一致（字段、顺序、数量）
- [ ] run / rating / delete / manual-run / rerun 五个端点功能正常
- [ ] 旧 v1 文件中仍有数据时，detail 懒迁移正常工作
- [ ] 无并发写入问题
...
- [ ] 采样/RAG/校验步骤展开时显示照片缩略图网格
- [ ] 缩略图 hover 显示文件名/描述
- [ ] 编辑直觉步骤时使用表单而非 JSON 文本框
- [ ] 重跑时显示阶段进度（Stage 1/3 → Stage 2/3 → Stage 3/3）
- [ ] 重跑失败时显示错误信息（替代"请求失败"）
- [ ] 照片加载失败时降级显示 photo_id 文本
...
- [ ] 选照片 + 填直觉 → 生成的选题照片全部来自用户所选照片池
- [ ] 仅填直觉 → 行为不变（跳过 Stage 1，走 RAG）
- [ ] 仅选照片 → 行为不变（走完整管线）
- [ ] Step 2 界面上有明确的照片-直觉关系说明
...
- [ ] 可选择两个版本进行对比
- [ ] 文本差异（标题/角度/理由）以 diff 形式高亮展示
- [ ] 照片序列差异展示（新增/移除/叙事角色变化）
- [ ] 退出对比后恢复步骤列表视图
- [ ] 只有一个版本时，对比按钮不可用（灰色提示）
...
- [ ] 每次 run 随机采样恰好 3 张照片
- [ ] Stage 1 LLM 输出恰好 1 个直觉
- [ ] Stage 3 LLM 输出恰好 1 个提案
- [ ] 最终写入恰好 1 条历史记录
- [ ] 手动选题：用户自定义照片数量不受影响
- [ ] 备选路径：不受影响
- [ ] 历史列表中不再出现多条记录共享同一 trace_id 的情况
...
- [eval-3.8-2026-07-31.json](../../data/eval_reports/eval-3.8-2026-07-31.json) — 3.8 评估报告
