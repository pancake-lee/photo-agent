...
- **方式 1：服务器目录批量处理** — 原图已在服务器上（如 `/root/project/photos/`），用户熟悉技术。流程：Web 配置源目录 → Go 后台异步扫描 + 批量 VLM → 前端轮询进度 → 完成后刷新图片列表
- **方式 2：Web 交互上传** — 原图在用户本地电脑，用户不熟悉技术。流程：前端选择文件 → 前端压缩 → 上传到 Go → Go 存储 + 触发 VLM 描述（单张同步或小批量异步）
...
- **方式 2（Web 上传）**：后端收到上传文件后，检查 `Storage.PhotoPath` 中是否已有同名文件
...
6. **批量导入 source_dir 范围限制**：是否限制为 `config.yaml` 中 `Storage.PhotoSrc` 的子目录？安全考虑。（Phase 2 再议）
