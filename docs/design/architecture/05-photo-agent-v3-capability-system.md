...
首个非发帖压力测试选定为“照片跨期对比”：它复用范围解析、检索和详情获取，新增基于两组证据形成结论的 Skill，交付对比报告而非帖子。它能直接检验 V3 的复用主张；照片整理、废片标记等会引入写操作或新的治理规则，不作为本轮验证对象。

主题发现是已稳定的 Workflow。V3 只为它补可调用的范围输入、结构化输出和失败语义，保留三阶段内部控制流；不将其重写成由 Runtime 逐步决定的聊天 Pipeline。

### 兼容性边界

- 社交发帖保留既有权威范围、连拍折叠、候选交付和文案事实依据等可靠性成果。
- 普通 SQL、RAG、Combined 仍走直接路径；只有需要跨步骤收集事实和产物的请求才进入 Runtime。
- 目标或交付变体不得从自然语言关键词的字面形式推断，必须来自入口识别或显式结构化选择。
- 本轮不引入 Planner、Memory、能力检索、Multi-Agent 或新的照片写操作；这些属于后续 V4/V5 或新的独立需求。
...
V3 的退出条件是能力边界稳定且可观察：社交发帖与跨期对比两种目标能复用同一 Runtime 和基础能力，主题发现可作为 Workflow 独立调用，且指标证明没有以新增端到端 Pipeline 换取成功。此时 Planner 才知道系统真正“会什么”，V4 的计划才可能可执行。
