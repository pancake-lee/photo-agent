...
- 语义质量进入检查环：文案事实依据由语义评估把关，不通过时带反馈修复（有界），修复耗尽以质量终态停止，不伪装完成；选片采用连拍折叠、范围归属和 ID 去重的确定性保护，不以文字摘要推断画面近重复
...
> 2026-09-06 修订：选片评委只有文字摘要、无法查看实际画面，真实山西请求中连续将不同古建角度和玩偶特写/人物合影误判为近重复，造成恢复预算耗尽。因此选片代表性不再作为阻断质量门；连拍折叠、范围归属和 ID 去重继续作为确定性保障，选片提示仍要求优先场景多样性。
