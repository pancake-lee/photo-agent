...
## AI 资产质量闭环：审查同步回写方案（2026-08-27，未采用）

**未采用方案**：每次打开批量审查或详情时，重新运行质量规则并把通过/异常结论回写至照片状态，再由 Embedding 读取该状态。

**未采用原因**：只读审查产生隐式写入，且任一读取入口漏做同步仍会造成状态分歧。当前质量结论应由当前描述即时计算，持久化仅保留 VLM/Embedding 的实际处理结果和版本事实。

## AI 资产质量闭环：仅描述质量实时、向量状态持久化方案（2026-08-27，已撤回）

**撤回方案**：描述质量即时校验，但保留 SQLite 中的 Embedding 成功、过期和失败状态作为详情、批量审查与检索准入依据。

**撤回原因**：`DSC_0051.jpg` 证明旧 `stale` 值仍会把正常照片显示为异常。向量是否存在及是否对应当前描述同样可由 Chroma 实时检查，不能保存为可能滞后的照片状态。

## AI 资产质量闭环：重型修复工作区方案（2026-08-27，已否定）

**被否定方案**：新增“AI 问题”工作区、独立修复任务、批量选择、停止续跑、人工确认描述可用与治理报告。

**否定原因**：根因是错误请求图片导致 VLM 描述和后续向量污染；正确修复是对异常照片重跑既有 VLM + Embedding 流程。为此新增一套用户工作区和任务系统会扩大代码量、交互复杂度与维护成本，且重复已有缩略图状态、照片详情和顶部批量队列能力。

**替代决策**：保留调用前/调用后的本地校验；复用既有 VLM/Embedding 状态图标展示感叹号和原因，详情页原地重新生成，顶部批量按钮在确认前汇总待处理与疑似异常候选并展示一行缩略图。详见 [轻量修复交互设计](design/2026-08-27-1-ai-repair-interaction-design.md)。
