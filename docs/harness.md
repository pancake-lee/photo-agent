...
flowchart TD
    ...
    K -->|是| L["待用户验收<br>仅限 AI 无法自动执行的最后一环"]
    L -->|用户确认即终态| M["回写 Done<br>不反向核验"]
    M --> F
    F --> G["代码质量评估"]
    F --> H["功能效果评估"]
    F --> I["启发式规则引擎"]
...
任务状态用来说明当前负责人，而不是笼统记录“还没结束”。`WIP` 只表示 AI 仍在开发或自动验证，`待用户验收` 表示 AI 已完成所有可执行工作、仅剩一项用户在真实环境中的最后操作，`Done` 表示验收已完成（AI 自动验证在交付前完成，用户人工确认即终态）。
