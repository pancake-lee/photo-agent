...
flowchart TD
    ...
    D --> E
    E -->|产出代码与自动验证| K{"需要用户验收？"}
    K -->|否| F["评估模式<br>（Evaluate）"]
    K -->|是| L["待用户验收<br>明确操作与预期"]
    L --> M["AI 核验 Trace/日志<br>回写 Done"]
    M --> F
    F --> G["代码质量评估"]
    F --> H["功能效果评估"]
    F --> I["启发式规则引擎"]
...
任务状态用来说明当前负责人，而不是笼统记录“还没结束”。`WIP` 只表示 AI 仍在开发或自动验证，`待用户验收` 表示 AI 已完成所有可执行工作、正在等待用户做一项真实环境判断，`Done` 表示验收和证据记录均已完成。

生成器必须主动提出 `待用户验收` 的单一操作和预期结果。用户确认后，生成器负责查阅已有 Trace、日志或持久化数据，并在同一轮将任务回写为 Done；自动产生的记录不是用户需要导出或提交的作业。完整规则和 backlog 模板见 [工作模式手册](handbook/work-modes.md#任务关闭与用户验收)。
...
- **角色间 Handoff**：通过 backlog 条目结构化字段传递上下文，无需依赖对话历史。规划产出、子任务与实施步骤均写入同一 backlog 条目，评估发现写入新条目；不创建独立任务计划文档
