...
`make start` 内部已含编译（`make dev`），无需单独编译。backend（10004）、agent（10005）、web（10006）三个服务需全部 HTTP 健康。
...
对 `docs/eval/baseline.md` 中的 P@10 与 MRR，较历史基线下降超过 10% 时标记为显著下降，并在评估报告和 backlog 中记录。
...
- 第一步补充：若评估目标为 `backend/`，运行 `GOTOOLCHAIN=local go run ./tools/go/main.go --self-check`，以输出列明评估文件、数据状态和至少一条关键用户用例
- 第三步补充：后端专项评估必须覆盖《后端代码质量评分标准》的八项维度，并记录加权总分、阻断项、测试分层选择及用户用例闭环
- 第四步补充：评估报告双格式的完整规范见 [2026-07-26-1-harness-design.md](../design/2026-07-26-1-harness-design.md)「2026-08-21 评估报告双格式规范」节
