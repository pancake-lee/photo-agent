...
6. **验收关单（必须逐项完成，不可跳过）**：
   ...
   - [ ] 必须在浏览器、部署环境或真实业务场景判断的验收，将任务改为 `待用户验收`。在 backlog 任务详情中写出 `（用户）` 操作、预期结果、最小回复方式，以及 AI 将读取的 Trace、日志或持久化证据。
   - [ ] 最终回复中主动、醒目地请求这一个用户验收，不将“待验收”埋在变更摘要中。
   - [ ] 用户确认后，AI 先自行读取已有证据并在同一轮更新任务为 `Done`、同步总览和相关中枢文档；只有没有自动证据来源时，才请求用户提供最小必要结果。
...
4. 如仅剩用户在真实环境中的判断：更新为 `待用户验收`，在最终回复中给出单一、可执行的 `（用户）` 操作、预期和最小回复方式；用户确认后由 AI 核验证据并关单
...
- `待用户验收`：代码和全部 AI 可执行验证已完成，唯一剩余步骤是用户在真实环境中的判断。
- `Done`：所有验收已完成。用户确认过的运行时结论，优先由 AI 从 Trace、日志或持久化数据核验后记录。
...
```markdown
- **（用户）验收操作**：打开 <入口>，执行 <一个操作>。
- **预期结果**：<可观察的通过条件>。
- **最小回传**：回复“<任务编号> 已通过”；无需导出已有系统日志。
- **AI 关单证据**：AI 检查 <Trace/日志/持久化数据位置> 后更新 Done。
```

用户已在对话中说明验收完成时，AI 必须将其视为关单信号，主动查验上述证据并更新 backlog，不得仅解释状态原因或要求用户代为收集系统已有记录。
