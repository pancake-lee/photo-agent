...
2. **必须列出方案要点并等待用户确认**（方案有多个选项时提供选择，仅一个选项时简述后确认），不可跳过确认直接进入生成
3. 确认后更新 backlog 条目状态为「已规划」
...
3. 如全部验收通过：更新 backlog 条目状态为「Done」，简要总结改动清单和测试结果
4. 如发现问题：在对话中指出，让用户决定是立即修复还是留到下一轮

**行为约束**：

- 三个阶段在同一会话中连续执行，无需 `/clear`
- 阶段 1 的确认是唯一阻塞点（用户说"可以"后自动进入阶段 2+3）
