...
5. **必须输出明确的任务列表，且任务拆分的落点是 backlog 条目**，不可将待执行任务埋在大段文字中间，也不可把任务拆分/实施步骤写进 design 文档或中枢文档（后者只做方向性指引与索引）。用户需手动执行的任务以 **（用户）** 前缀着重标记，例如："（用户）在 `.local/my-config.yaml` Go 段添加 `Embedding.APIKey`"
