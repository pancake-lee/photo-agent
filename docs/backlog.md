...
| 已规划 | 列表浏览   | LB4  | 导航跳转不准确（坐标 + 锚点）  |      |
| 已规划 | 时间线     | LB5  | 散片自动时间线 + 时间线管理页 |      |
| 已规划 | 列表浏览   | LB6  | 图片管理顶栏重排（单行 + 弹出收纳） |      |
...
- **验收**：
  - [ ] 分割线按天/月/活动正确插入；连拍组折叠不影响分割线
  - [ ] 右侧导航正确高亮跟随；点击跳转（已加载锚点滚动 / 未加载重置拉取）正确
  - [ ] 按天方式下导航降为月份
  - [ ] 滚动加载追加照片，分页组件移除
  - [ ] 「未分类」「未知时间」段落落位正确；timeline="none" sentinel 生效
  - [ ] 排序 UI 收敛为升降序切换
  - [ ] 2000+ 张滚动流畅
...
- **验收**：
  - [ ] 点击导航后目标段首屏落位准确（首张照片在视口顶部附近），高亮与目标段一致
  - [ ] 滚动时高亮实时跟随当前段，无滞后一段的现象
  - [ ] 连拍折叠（精细/模糊）下同样准确
...
- **方案**：
  ...
  - 存储：timeline 事件迁数据库表 `timeline_event`（不建外键），启动时检测表空且 JSON 存在则一次性导入，JSON 退役
  - 接口：TimelineService 扩展 CRUD + RecomputeTimelines（异步 + 进度轮询，复用 RebuildBurstGroups 模式），走 proto codegen 管线
  - 前端：`/timelines` 独立管理页（事件增删改 + 重算进度 + 散片组只读展示），图片管理筛选数据源改 ListEvents，保留「未分类」sentinel 兼容未重算的存量照片
- **分析**：与用户确认的三个决策点：散片粒度（活动间隔内按月切）、重算保留人工值（加 timeline_manual 列）、事件迁数据库表（放弃 JSON）。散片名是自动产物，重算整组刷新，不承诺稳定。agent 侧 timeline SDK 无实际调用方，重生成即可
- **验收**：
  - [ ] 散片按规则正确生成，活动分段导航中每个散片组有锚点可跳转
  - [ ] 管理页事件增删改保存生效，重算进度可见，完成后照片 timeline 刷新
  - [ ] 人工改过的 timeline 在重算后保留
  - [ ] JSON 一次性导入成功，旧 timeline.json 退役
  - [ ] 图片管理活动筛选下拉含散片组，未重算存量照片仍可筛「未分类」
  - [ ] `go test` + `pnpm build` 通过
...
- **验收**：
  - [ ] 顶栏单行，宽度不足时优先压缩标题与搜索框，不换行不堆叠
  - [ ] 弹出面板内所有原功能可达，操作即时生效
  - [ ] 筛选激活时触发器有角标提示，重置可清空
  - [ ] 布局符合 docs/ui-rules.md（8px 网格、邻近性、左信息右操作）
  - [ ] `pnpm build` 通过
