...
- **严重程度**：P1，回归风险不可定位。
- **证据**：[后端代码质量基线评估](../data/eval_reports/2026-08-29-backend-code-quality-baseline.md)。
- **方案**：以临时 SQLite、临时文件目录和进程内 HTTP 服务建立可重复的关键用户用例。BQ2 以“启动迁移不含破坏性 DDL”的静态和活动库证据收口，不在测试中改表结构；其余用例从公开 Service 或 HTTP 行为验证最终数据库、文件、队列状态和 ZIP 内容。上传/删除若测试发现多资源部分失败，会直接修复创建后清理、删除前暂存或失败回滚的根因，确保失败可见且不遗留静默不一致。
- **实施任务**：
  - 上传与删除：覆盖 JPG、NEF 和代表性的下游失败，断言源文件、缩略图与照片记录的最终一致性。
  - VLM 队列：以可替换的本地 VLM 调用覆盖启动、停止、成功、失败和状态/处理履历回写，不访问云端服务。
  - 草稿：通过进程内 HTTP 覆盖创建、更新、列表、详情、删除与 ZIP 导出，检查 Markdown 和入选原图内容。
  - 迁移：在 BQ2 验证中确认无旧列删除 DDL；测试始终使用已建好的临时数据库，遵守“不在测试改表结构”约定。
  - 验证：运行新增用例、全量 Go 测试、`go vet`、受影响 Service/DAO 的 race 检查，并记录每条用例的最终状态。
