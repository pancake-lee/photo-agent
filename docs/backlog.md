...
- **实施记录（2026-09-01）**：
  - 约束解析：resolve_trip 扩展为约束解析能力。一次 LLM 调用抽取时间线/天序/时段/软提示；程序校验（时间线沿用确定性名称匹配，天序只认 first/last/ISO 日期，时段查固定映射表转小时窗，非法值按“无该约束”处理不终止）；范围 SQL 由程序只按硬约束拼装（不经 LLM）并物化为权威范围写入任务状态。无硬约束时范围标记不受限（全库）；时间线被提及但匹配不上维持 trip_unresolved 终态。
  - 时区修正：真实库 shot_at 混合 UTC(+00:00) 与本地(+08:00) 偏移，直接取小时会跨格式漂移。范围 SQL 的天序与小时窗统一经 `localtime` 修饰符换算本地时区（真实库验证：贵州第一天傍晚 UTC 小时误判 0 张，修正后 10 张）。
  - 归约层：新增 scope_materialized 观察，物化范围并确认“locate”里程碑（语义改为“确认候选范围”）；候选类观察一律与范围求交集，软提示零命中的时候选回落整个范围；范围晚于检索建立时既有候选同样回到范围内；受限但 0 张进入 empty_scope 确定性终态（不选片、不写文案，回复说明条件与放宽建议）。
  - 检索收紧：hybrid_search 删除“SQL 为空/过宽/交集为空 → 全库 RAG”替代语义，交集为空时候选交由权威范围兜底；sql/rag 语义不变，交集由归约层统一强制。
  - 交付阻断：select_photos 对 LLM 挑选结果做范围归属校验（范围外入选 selection_out_of_scope 可解释失败）；完成检查 selected_photos 要件在范围受限时增加“入选 ⊆ 权威范围”判定。
  - 提示词与翻译：decide 规则改写为“硬约束已由范围生效、检索 query 只写软提示”，移除旧的“已确认事实写进检索 query”注入点；过程面板 resolve_trip 标题改为“确认候选范围”，观察事实携带范围条件、数量与软提示用途，范围 SQL 可在执行细节中查看。
- **AI 自动验证**：Agent 全量单测 206/206 通过（新增 18 个：范围 SQL 拼装与时区换算、约束校验边界、软提示零命中兜底、范围外检索结果被交集过滤、范围外入选阻断、完成检查范围判定、空范围终态不选片不写文案、循环级山西约束保护）；用运行中的 Go 后端验证范围 SQL 真实可执行（山西第一天 80 张、第一天傍晚 44 张、最后一天傍晚 0 张，空范围路径有真实输入）。
- **（用户）验收操作**：重启 Python Agent（`make dev`）后，在聊天页发送「找山西旅游第一天傍晚的照片并生成发布文案」；再发送「找山西旅游最后一天傍晚的照片并生成发布文案」（真实库该范围为 0 张，验证空范围终止）。
- **预期结果**：第一个请求的执行过程首步为“确认候选范围 → 已确认候选范围「山西第一天傍晚」，共 44 张”，后续检索 query 只含软提示、太原植物园等词不再清空候选，最终入选照片全部属于该范围，执行细节中的范围 SQL 只含时间线/最早日期/17–19 点；第二个请求明确回复未找到符合条件的照片并给出放宽建议，不出现入选照片或虚构地点文案。
- **最小回传**：回复“AR9 已通过”，或贴出异常的回复文本/过程面板步骤。
- **关单方式**：用户回复确认后，AI 在同一轮将任务改为 `Done` 并注明确认日期，不追加核验。
- **关单记录（2026-09-01）**：用户确认 AR9 验收通过。
