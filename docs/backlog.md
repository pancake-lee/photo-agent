...
| Done    | 缺陷修复 | B8   | Stage 3 LLM photo_id 无校验（幻觉风险） |
| Done    | 缺陷修复 | B9   | 三阶段选题时间跨度约束未强制执行 |
| Done    | 工程     | R6   | suggest.py 内联 import re 提升到模块级别 |
| Done    | 缺陷修复 | B10  | suggest.py 全链路 shot_at 按时间戳处理导致日期逻辑失效 |
...
| Done    | 缺陷修复 | B11  | 星级评分无法取消（hover 遮盖） | 8.4 |
...
> 📋 中枢文档：[2026-07-27-topic-discovery-hub.md](design/2026-07-27-topic-discovery-hub.md) — 本专题全部关联设计/评估/backlog/提交的汇总入口

- **状态**：Done（核心方案已实现，衍生问题 B8/B9/R6 待规划）
- **背景**：主题发现（`suggest.py`）产出的选题建议中，每组照片都是时间接近、场景接近、风格接近的高相似度组合。不符合功能设计初心——发现用户在图库中没轻易看到的角度。
- **方案**：[2026-07-27-2-suggest.md](design/2026-07-27-2-suggest.md)——三阶段编辑视角提案：① 随机采样 → LLM 主题直觉；② RAG + 多样性约束扩展选片；③ LLM 提案。原有三维度保留为备选回退路径。
- **分析**：核心矛盾是算法在找共同点，但选题发现的价值在对比/连接。经历三轮评估闭环：首次 4.2/10（发现 B4/B5/B6/R5）→ 修复后二次 5.8/10（发现 B7 配置缺失）→ 修复后三次 7.0/10（主路径首次跑通，发现 B8/B9/R6）。详见中枢文档。
- **验收**：
  - [x] 选题提案中至少包含 5 张照片（代码侧 `_STAGE2_MIN_PHOTOS = 5` 约束）
  - [ ] 照片时间跨度 > 7 天（1/3 不满足，约束仅日志输出未强制执行 → B9）
  - [x] 叙事角度描述可被理解为有发布价值（Stage 3 LLM prompt 要求叙事角度 30-60 字）
  - [ ] Stage 3 LLM 返回的 photo_id 全部有效（已证实幻觉 1 次 → B8）
...
- **状态**：Done
- **背景**：当前选题生成结果仅在前端 Vue ref 中临时持有，刷新页面后消失。无法回溯历史选题、无法比较/打分。用户需要持久化存储以便对比评估不同批次的选题质量。
- **方案**：Agent 层 JSON 文件存储（`agent/data/suggest_history.json`），新增 5 个 API（history 列表/详情/删除 + rating 更新 + 现有 run 接口改为保存），前端改为历史列表 + 展开查看模式，每条结果支持五星打分和删除。详见设计文档。
- **分析**：-
- **验收**：
  - [x] 生成选题后刷新页面，历史结果仍在
  - [x] 历史列表按时间倒序排列
...
- **方案**：在 `_stage3_generate_proposals` 中构建全库 + 扩展候选集的 `valid_ids` 集合，对 LLM 返回的每个 photo_id 逐一校验。无效 ID（不存在或重复）从扩展候选列表中取未使用的真实 ID 替换。校验后不足 5 张时自动补充。
- **分析**：B10 修复后重新评估（2026-07-28，总评 8.0/10），B8 在 3 轮 9 提案中正确捕获 12 次幻觉（均为 LLM 返回数字索引 '0'-'23' 而非 UUID），全部替换为有效 ID。最终 48/48 photo_id 有效。
- **验收**：
  - [x] Stage 3 返回的 photo_ids 全部经过数据库存在性校验
  - [x] 无效 ID 有明确的修复策略（替换而非丢弃）

### B9 三阶段选题时间跨度约束未强制执行

- **状态**：Done（2026-07-27 实现，B10 修复后全面生效）
- **背景**：2026-07-27 第三次评估 3.4 时发现两个时间跨度问题：(1) '水边的独白' 6 张照片仅跨越约 22 小时（Feb 2-3），不满足设计方案验收标准「> 7 天」；(2) '人鸥之间' 中 3 张海鸥照来自同一拍摄会话（同一上午 21 分钟内）。Stage 2 多样性约束（`_STAGE2_MAX_PER_DATE=2`）保证了 RAG 候选的日期分散，但 Stage 3 LLM 在从 30 张候选中选择最终照片序列时不受时间跨度约束。
- **方案**：三层修复 — (1) `_STAGE3_SYSTEM_PROMPT` 增加 ≥7 天硬性要求；(2) `_build_stage3_prompt` 展示每张候选照片的拍摄日期；(3) 后处理阶段计算 photo_sequence 时间跨度，不足 7 天时从扩展列表最早/最晚日期替换。
- **分析**：B10 修复后重新评估（2026-07-28，总评 8.0/10），三层机制全部正常工作。prompt 层 + LLM 可利用可读日期后效果显著，9/9 提案 Stage 2 跨度 144~420 天，B9 后处理 date_map 正常构建，所有提案天然满足 ≥7 天要求（后处理替换逻辑未触发）。
- **验收**：
  - [x] Stage 3 prompt 包含时间跨度硬性要求
  - [x] 候选照片日期信息展示在 prompt 中
  - [ ] 每条选题的照片时间跨度 ≥ 7 天（1/3 不满足，受 B10 阻断）

### B11 星级评分无法取消（hover 遮盖）

- **用户原始描述**：星星有三种状态的展示，一种是空心的星星，一种是实心的星星表示 hover 的状态，而新增一种实心的星星，并且带有发光的效果，用来表示已点亮的状态。而已点亮三颗星，当 hover 到五颗星的时候，前三颗星是发光的状态，而后两颗星是实心不发光的状态。已点亮三颗星，当 hover 到两颗星时，则只需要把第三颗星灭掉，变成空心的状态。已点亮三颗星，当 hover 到三颗星时，所有星星都灭掉，hover 上去会有3分的提示，这个提示变成取消。
- **状态**：Done
- **背景**：用户给主题打 3 星后，无法取消评分到 0 星。点击已点亮的星星时视觉上无变化（hover 状态遮盖了实际评分变更），用户无法感知评分已被清除。
- **方案**：星星改为三态（空心/实心/发光），用纯 CSS 区分：空心为灰色、实心为橙色、发光为橙色 + scale(1.3) 放大 + 三层 text-shadow 光晕。所有星统一用 `★` 字符，`inline-block` + 固定宽度防止 UI 跳动。hover 到当前评分时全部灭掉 + tooltip "取消"，点击时清除 hover 避免视觉遮盖。改动仅限 `SuggestView.vue` 前端一个文件。
- **验收**：
  - [x] 点击已点亮的星星，视觉上立即显示所有星星灭掉（`handleRate` 开头 `hoveredStar.value = null`）
  - [x] 可正常切换 1-5 星和 0 星之间任意分值（toggle 逻辑 `item.rating === star ? 0 : star` 未变）
  - [x] hover 到当前评分时 tooltip 显示"取消评分"（需用户启动 dev server 验证）
  - [x] 三态视觉效果正确：发光（已确认）/ 实心（预览）/ 空心（无）（需用户启动 dev server 验证）
- **评估**：8.4（正确性 9 健壮性 8 可维护性 8 简洁性 9 准确性 9 完整性 8 一致性 9 交互体验 8），详见 [eval-B11-2026-07-28.json](../data/eval_reports/eval-B11-2026-07-28.json)

### R6 suggest.py 内联 import re 提升到模块级别

- **状态**：Done（2026-07-27 实现）
- **背景**：R5 合并 JSON 解析逻辑后，`import re` 仍在 `_parse_llm_json_response` 和 `_parse_proposal_response` 两处函数体内联。作为项目公共函数，提升到模块顶部更符合 Python 惯例。
- **方案**：将 `import re` 移到模块顶部（第 28 行），删除两处函数体内联 import。
- **分析**：问题不影响功能，属于代码风格 cleanup。`re` 模块是 Python 标准库，提升到模块顶部无任何副作用。
- **验收**：
  - [x] `import re` 提升到模块顶部，删除函数体内的两处内联 import

### B10 suggest.py 全链路 shot_at 按时间戳处理导致日期逻辑失效

- **状态**：Done（2026-07-27 实现）
- **背景**：2026-07-27 评估 B8/B9/R6 时发现，Go 后端 API 返回的 `shotAt` 字段是 Unix 时间戳字符串（如 `"1780733203"`），但 suggest.py 中所有日期处理代码均按 ISO 日期字符串（如 `"2025-05-02"`）操作。具体影响：(1) Stage 1 随机采样按日期分组失效（实际按时间戳前 10 位分组，基本等同按秒分组）；(2) Stage 2 日期跨度计算始终为 0（`datetime.date.fromisoformat("1780733203")` 始终抛 ValueError 被静默捕获）；(3) Stage 3 prompt 中传给 LLM 的日期是无意义的数字，LLM 无法据此做日期多样性判断；(4) B9 后处理的 date_map 始终为空，导致时间跨度强制约束静默跳过。
- **方案**：新增 `_parse_shot_date` 工具函数统一处理 Unix 时间戳和 ISO 日期字符串。函数先尝试 `datetime.fromtimestamp(int(val))`，失败后回退到 `datetime.date.fromisoformat(val[:10])`。全链路 8 处调用点统一替换：`_random_sample_photos`（2 处日期分组 + 2 处日志）、`_stage2_expand_selection`（日期分组 + 时间跨度计算）、`_build_stage3_prompt`（LLM 日期展示）、B9 post-processing date_map、`_find_temporal_patterns`（2 处）。
- **分析**：Go 后端 SQLite 中 shot_at 存储为 ISO 8601 格式，但 API 序列化时被转为 Unix 时间戳。Python 侧统一转换是改动最小的方案。
- **验证**：Stage 1 日期分组从 1156 个伪日期降至 64 个真实日历日。Stage 2 时间跨度从 0 天升至 273~349 天。B9 date_map 正常构建，3/3 提案天然满足 ≥7 天要求。B8 幻觉 ID 本轮 0 个。
- **验收**：
  - [x] Stage 1 日期分组按日历日期而非时间戳前缀
  - [x] Stage 2 日期跨度计算正确
  - [x] Stage 3 prompt 中日期信息对人类和 LLM 可读
  - [x] B9 后处理 date_map 有效，时间跨度强制约束可正常执行
