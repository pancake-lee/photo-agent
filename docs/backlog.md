...
| Done   | 列表浏览   | LB4  | 导航跳转不准确（坐标 + 锚点）  |      |
| Done   | 时间线     | LB5  | 散片自动时间线 + 时间线管理页 |      |
| Done   | 列表浏览   | LB6  | 图片管理顶栏重排（单行 + 弹出收纳） |      |
...
- **验收**：
  ...
  - [ ] JSON 一次性导入成功，旧 timeline.json 退役（（用户）启动后端看日志导入条数）
  - [ ] 图片管理活动筛选下拉含散片组，未重算存量照片仍可筛「未分类」
  - [x] `go test` + `pnpm build` 通过
