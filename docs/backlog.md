...
| 已规划 | Agent 升级 | AR8   | Runtime 多步执行缺少用户可理解的过程反馈     | 8.31 |
...
- **（用户）验收操作**：重启 Agent 与前端后，在聊天页发送“找山西旅游第一天的照片并生成发布文案”，等待期间展开“执行过程”，完成后刷新该会话；再发送一个普通 RAG 查询。
- **预期**：Runtime 步骤按真实执行持续更新、与轨迹顺序一致，技术细节不泄露内部日志/ID；最终答案与照片正常，刷新后过程仍在；普通查询没有过程面板。
...
- **状态**：已规划（2026-08-31）
- **专题中枢**：[Agent Runtime 专题中枢](design/2026-08-31-2-agent-runtime-hub.md)
- **问题记录**：[Runtime 检索约束幻觉问题与修复设计](design/2026-09-01-1-runtime-constraint-hallucination.md)
...
- **方案**：详见问题记录。Runtime 将原始目标拆为硬约束与软叙事提示：已确认的时间线、第一天和时间段建立权威候选范围；地点、景物、氛围等未结构化或仅补充的信息只用于该范围内排序、选图说明和文案。硬约束查询为空时以“未找到符合条件的照片”确定性结束，禁止以全库 RAG 替代；RAG 只可在权威候选范围内排序。完成检查在选图和写文案前后验证候选与入选照片的范围归属，不能满足时不生成带确定性地点/场景断言的文案。
- **实施任务**：
  - 建立 Runtime 目标约束分层和权威候选范围：旅行、日期和时间等可验证条件必须先落为范围；软提示不参与范围过滤，除非用户明确要求其为严格条件且库中存在对应可验证字段。
  - 收紧 SQL/RAG/混合检索能力：空范围、交集为空或 RAG 失败均保留硬约束并进入可解释终态；RAG 仅对范围内候选排序，不再能替代范围。
  - 扩展 Runtime 状态、进度翻译和完成检查，记录候选范围来源与约束验证结果；空范围或范围外入选必须阻断选片/文案交付，并向用户说明缺口。
  - 为山西用例及 SQL 为空、RAG 返回范围外结果、范围内 RAG 重排、选片/写文案前约束复核补充确定性测试；新增真实环境验收检查最终照片与时间线/首日/时间段一致。
