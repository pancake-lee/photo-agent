...
- **状态**：已规划
- **背景**：主题发现（`suggest.py`）产出的选题建议中，每组照片都是时间接近、场景接近、风格接近的高相似度组合。虽然主题高度切合，但不符合功能设计初心——发现用户在图库中没轻易看到的角度。根因是三个发现维度（高频未成组/时间线规律/稀缺优质）都遵循"按共享属性值匹配照片"的范式，必然选中同一次拍摄的相似照片。用户自己做完拍摄活动后自然会做这样的归类，不需要 AI 辅助。
- **方案**：[2026-07-27-suggest.md](design/2026-07-27-suggest.md)——核心思路：从"共享属性选片"转变为"编辑视角提案"。三阶段工作流：① 随机采样 6-9 张照片让 LLM 以编辑视角产生主题直觉（引入偶然性）；② 围绕主题方向通过 RAG + 多样性约束扩展选片；③ LLM 沉淀完整选题提案（标题 + 叙事角度 + 照片序列 + 发布理由）。现有三维度保留为备选输入。
- **分析**：2026-07-27 用户人工评估确认问题。当前约 1177 张照片，三个维度都把"共享某属性值"作为候选生成逻辑，LLM 没有机会看到跨上下文的连接。核心矛盾：算法在找共同点，但选题发现的价值在对比/连接。
- **验收**：
  - [ ] 选题提案中至少包含 5 张照片
  - [ ] 照片时间跨度 > 7 天（不来自同一连续拍摄会话）
  - [ ] 叙事角度描述可被理解为有发布价值
