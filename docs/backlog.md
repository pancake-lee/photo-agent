...
| Done   | BQ6      | BQ10 | 用户路径测试修改表结构            |
| Done   | BQ6      | BQ11 | VLM 队列缺少端到端自动闭环        |
...
| Done   | 配置治理 | CFG2 | 配置类与索引脚本契约不一致          |
| Done   | 配置治理 | CFG3 | 核心模板保留无消费者的 Dify/VLM 配置 |
| Done   | 配置治理 | CFG4 | 数据库与运行目录配置职责边界不清    |
| Done   | 配置治理 | CFG5 | 配置值缺少统一的类型范围和枚举校验  |
| Done   | 配置治理 | CFG6 | 价格模板占位且价格单位契约不清      |
