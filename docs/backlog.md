...
- **状态**：Done（2026-09-07，用户确认），依赖 AR2-1 至 AR2-6
...
- **用户验收失败记录（2026-09-06）**：真实请求「找山西旅游第4天的照片并生成发布文案」的 trace `d36e7fe3f912`：`write_post` 首次及两次携带反馈的重试均被事实核查评委拒绝，最后因 `repair:write_post` 恢复预算耗尽终止（`quality_gate_failed`）。评委的证据输入仅保留摘要字段，却否定了生成文案中来自完整视觉描述或用户目标的事实（如佛像手势、山西/第四天）；生成器和评委的事实边界不一致，导致可恢复的改写被持续拒绝。
- **修复方案（2026-09-06）**：文案事实评委复用图文工坊实际传给生成模型的照片上下文，保证可用的视觉事实集合一致；同时单列原始用户请求为允许使用的上下文事实，评委不再要求旅行名称、相对日期等用户已明确提供的信息必须由单张照片证明。评委仍只接受上述两类证据，未被照片上下文或用户请求支持的具体事实继续拒绝。
- **任务列表**：
  1. 让文案事实评委构造与图文工坊一致的照片证据，并随文案观察保留实际使用的风格。
  2. 在评委提示词中区分视觉事实与用户提供上下文，避免误判旅行/日期等请求事实。
  3. 固化真实山西失败路径的证据一致性回归，并保留虚构地点被拒绝的既有覆盖。
- **实施记录（2026-09-06）**：`evaluate_copy` 改为复用 `post_studio.build_photo_context` 构造照片证据，和 `write_post` 的视觉事实字段、全量/简略模式及风格保持一致；`write_post` 观察记录实际文案风格供评委还原同一上下文。评委提示词单列原始用户请求，并仅允许其中的旅行名称、相对日期等请求上下文，不再误判为必须由照片证明的事实。
- **AI 自动验证（2026-09-06）**：新增真实山西回归：评委提示词同时包含完整视觉细节「右手抬起掌心朝前」和用户请求「山西旅游第4天」，防止事实边界再次漂移；既有虚构地点拒绝与带反馈修复环仍通过。`python -m unittest discover -s tests` 299/299 通过。
- **回归修复（2026-09-06，第 2 次）**：trace `17f974291b1f` 证明剩余失败来自选片代表性评委，而非文案：其仅根据文字摘要，先将两张不同古建角度、后将玩偶特写与人物合影误判为近重复，三轮选片耗尽 300 秒预算。选片结果已由连拍折叠、候选范围和 ID 去重确定性保证；移除无法看见真实图像的阻断式选片评委，选片提示改为「优先不同场景、候选不足时少选不凑数」。文案事实评委保留，继续阻断无证据的虚构内容。
- **原始请求回归用例（2026-09-06）**：`test_shanxi_day4_conversation_regression_completes` 固定输入「找山西旅游第4天的照片并生成发布文案」，模拟时间线第 4 天范围、选片和文案生成完整循环，断言无 `terminal_reason`、无 `stop_reason` 并交付标题与照片。`python -m unittest discover -s tests` 300/300 通过。
- **回归缺口（2026-09-06）**：上述图级用例的照片、时间线和 LLM 均为 mock，不能证明真实山西候选与真实 LLM 的组合行为。后续须在独立目录新增只读真实数据/真实 LLM 回归用例，日常回归不执行；仅在用户明确授权的当轮调用，并记录实际消耗与结果。
...
- **状态**：Done（2026-09-06，用户确认）
- **背景**：用户需要把已选照片直接取回本地。目前图片管理的多选状态与对话输出的图片列表均缺少下载入口。
- **范围**：图片管理多选照片下载；对话输出图片列表下载。
- **方案**：采用服务端 ZIP 打包。后端接收有序照片 ID，先完整校验照片记录和原始文件，再将原文件流式写入响应，不创建临时 ZIP 文件；ZIP 内保留原始扩展名，并在重名时确定性追加序号。图片管理的选择工具栏和每条含照片的对话结果各提供一个下载按钮，均调用同一入口；请求失败时保留当前选择/结果并展示明确错误。
- **任务列表**：
  1. 提供照片 ID 批量下载接口，校验空值、重复/不存在 ID 和缺失原始文件，流式返回带安全文件名的 ZIP。
  2. 在图片管理多选工具栏接入 ZIP 下载并展示加载、成功和失败状态。
  3. 在对话结果的照片附件区接入同一下载操作；补充 ZIP 条目命名与前端请求的定向测试。
- **验收**：从图片管理下载已选照片、从任意含图片的聊天结果下载，均得到一个可解压的 ZIP；其中包含所有且仅包含目标原始文件，重名文件不互相覆盖；无效请求有明确错误提示。
- **实施记录**：新增 `POST /api/v1/photos/download`，对 ID 和原始文件完整校验后直接流式输出 ZIP，单次最多 200 张；ZIP 条目使用原始文件名，冲突时追加序号。图片管理多选工具栏和聊天照片附件区均已接入“下载 ZIP”按钮，并在下载过程中禁用重复提交、失败时展示错误。
- **（用户）验收操作**：启动后端与前端后，在图片管理中选中至少两张照片，点击“下载 ZIP”；再打开一条含照片的聊天结果并点击“下载 ZIP”。
- **预期结果**：两次均下载一个可解压 ZIP，且各自只含对应的原始照片文件。
- **最小回传**：回复“DL1 已通过”。
- **AI 自动验证**：`GOTOOLCHAIN=local go test ./internal/defaultService/service`、`pnpm test --run src/utils/downloadPhotos.spec.ts`、`pnpm build` 均通过；重名 ZIP 条目与前端失败提示已覆盖测试。
