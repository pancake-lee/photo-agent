...
| `_fetch_all_photos` 分页逻辑 | `embed_queue`, `auto_embed`, `evaluation`, `cluster`, `server` | 5 次 |
| 照片预览 Modal | `ChatView`, `GoldenQueryManagement`, `ClusterView` | 3 次 |
| 照片缩略图渲染（h 函数） | `GoldenQueryManagement`, `ClusterView` | 2 次（~80 行/次） |
| 日期格式化 | 各处 `new Date(t).toLocaleString('zh-CN')` | ~15 次 |
...
| LLM 调用超时 | 所有 `ChatOpenAI` 无 `request_timeout` |
| LLM 调用重试 | Text-to-SQL 和 RAG 模块绕过工厂函数，无重试/fallback |
