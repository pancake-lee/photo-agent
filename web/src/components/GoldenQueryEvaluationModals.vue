...
<template>
  <NModal :show="resultVisible" preset="card" title="黄金用例评估结果" style="width: 820px; max-width: 95vw;" :mask-closable="!evaluating" @update:show="emit('update:resultVisible', $event)">
    ...
    <div v-else-if="result" class="eval-result">
      <div v-if="!result.data_trusted" class="eval-data-warning">数据不可信：{{ result.asset_health.unhealthy || 0 }} 张期望照片的描述或向量未通过当前准入，本次分数不能作为检索质量结论。</div>
  ...
  <NModal :show="detailVisible" preset="card" :title="detail?.question || '评估明细'" style="width: 720px; max-width: 95vw;" @update:show="emit('update:detailVisible', $event)">
    <div v-if="detail" class="eval-detail-body">
      <div class="eval-detail-metrics"><span class="eval-detail-badge">P@{{ detail.effective_k || 10 }}: {{ (detail.precision * 100).toFixed(0) }}%</span><span class="eval-detail-badge">Recall: {{ (detail.recall * 100).toFixed(0) }}%</span><span class="eval-detail-badge">MRR: {{ (detail.mrr * 100).toFixed(0) }}%</span><span class="eval-detail-badge">检索 {{ detail.retrieved }} / 相关 {{ detail.relevant }}</span><span v-if="!detail.asset_health.trusted" class="eval-detail-warning">期望资产未全部健康</span></div>
...
.eval-detail-body { gap: 20px; }.eval-detail-metrics { display: flex; gap: 12px; flex-wrap: wrap; }.eval-detail-badge { padding: 4px 12px; border-radius: 4px; background: var(--n-color-embedded); font-size: 13px; font-weight: 500; }.eval-data-warning, .eval-detail-warning { color: var(--n-warning-color); background: var(--n-color-embedded); padding: 8px 12px; border-radius: 4px; font-size: 13px; }.eval-section { gap: 8px; }.eval-section-title { font-size: 14px; font-weight: 600; }.eval-section-sub { font-weight: 400; margin-left: 8px; }
