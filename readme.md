...
- Python 编码规范（Agent 目录）
  - 使用显式包名导入：`import langchain_openai as lc_openai`，调用时 `lc_openai.ChatOpenAI(...)`
  - 禁止 `from xxx import ClassName` 的直接类导入，确保代码中一眼能看出类的来源包
  - 包名过长时允许简写别名，但至少保留一个单词方便阅读，如 `import langchain.schema as lc_schema`、`import langchain.prompts as lc_prompts`
- 环境路径映射规则
