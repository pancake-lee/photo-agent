...
1. **规划产出**：Planner 将方案写入 backlog 对应条目的"详细说明"，状态更新为「已规划」
2. **启动执行**：用户在**新对话**中让 AI 读取「已规划」条目，进入生成模式执行
3. **交接内容**：backlog 条目本身即为交接协议——新对话中的 AI 只需读 backlog 即可理解要做什么、怎么做
...
编写或修改前端布局前，先读 `docs/design/ui-aesthetics-rules.md`，检查设计方案是否符合基本的规则，而仅逻辑上修改则无需关心这个文档。
