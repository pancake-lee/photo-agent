...
1. 读 `docs/eval/INDEX.md` 了解当前评估状态
2. 把问题结构化记录到 `docs/eval/feedback.md`（新增一条带状态的条目）
...
6. 修复完成后自检验证，更新反馈条目状态为「已关闭」，归档到 `docs/eval/archive/`

#### 规划模式

触发词：`规划一下` / `帮我设计` / `出个方案` / `怎么做`

1. 确认规划对象：用户口述的新需求、或 `docs/backlog.md` 中指定条目、或 `docs/eval/feedback.md` 中状态为「待分析」且类型为「效果问题」的条目
...
1. 确认执行对象：指定方案文档、或 `docs/eval/feedback.md` 中状态为「待修复」的条目、或 `docs/backlog.md` 中状态为「已规划」的条目
2. 读方案文档 + `docs/tech.md` + `CLAUDE.md` 编码规范
3. 按方案中的实现步骤逐步执行，每步完成后编译 + 测试自检
4. 更新相关工作追踪（feedback.md 状态 / backlog.md 状态 / todo.md）
...
| 5    | `docs/eval/INDEX.md`  | 评估器入口，反馈索引，基线指标                           | 技术方案、需求定义                                                  |
| 5    | `docs/eval/feedback.md` | 活跃反馈列表，角色间通过状态字段交接工作               | 技术方案、需求定义                                                  |
