# CLAUDE.md — Photo Agent

> 项目定位：AI 选题助手，让摄影照片库"会说话"。
> 技术栈：Go（Kratos v2+GORM+SQLite）+ Python（FastAPI+LangChain/Chroma）+ Vue 3（NaiveUI）。
> 开发模式：AI 辅助开发，用户作为"总指挥"。
> 公共规则块（harness marker）由 pancake/30-Tools/harness 统一同步，详见 [docs/harness.md](docs/harness.md)。
...
- **Design 文档门槛**：任务或需求需要记录的内容在 100 行以内时，不创建独立 `docs/design/` 文档；方案、任务和验收仅记录在 backlog，完成后随版本归档进入 `docs/archive/`。超过该篇幅且确有长期复用或跨轮串联价值时，才评估是否创建 design 文档
...
- **真实数据与 LLM 回归**：允许只读使用真实数据做回归，禁止修改；如测试必须写入，测试结束后必须恢复数据原貌。调用真实 LLM 的回归用例必须放在独立目录，日常自动回归不得执行；仅当用户在当轮明确授权时才可执行，并在调用前说明实际消耗。
...
- [docs/design/2099-01-01-future-requirements.md](docs/design/2099-01-01-future-requirements.md) — 未来需求暂存（从 backlog 审阅迁出）
