...
**模式间交接**：通过 backlog 条目结构化字段（状态 / 背景 / 方案 / 分析 / 验收）传递信息。用户使用 `/clear` 清空上下文后切换角色，AI 读取 backlog + 相关文档即可继续工作。`WIP` 只表示 AI 正在开发或自动验证，不得用于等待用户；仅剩真实环境判断时改为 `待用户验收`，写明 `（用户）` 操作、预期、最小回传和 AI 关单证据。用户确认后，AI 先自行检查 Trace、日志或持久化记录并在同一轮改为 `Done`；无用户操作的任务由 AI 自动验证后直接关单。
...
- **L1** `CLAUDE.md` — 全局协作规则、文档索引、工作模式触发。与 `AGENTS.md` 保持一致。禁区：产品需求、技术细节、任务进度
